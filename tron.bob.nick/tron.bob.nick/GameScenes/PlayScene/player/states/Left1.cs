...
namespace tron.bob.nick
{
    public class Left1 : DrawPlayer
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                float modulo = this.player.Position.X % 16;
                if (modulo <= this.player.Speed)
                {
                    int geheelAantalmalen16 = (int)this.player.Position.X / 16;
                    this.player.Position = (this.player.Position.X >= 0) ? new Vector2(geheelAantalmalen16 * 16, this.player.Position.Y) :
                                                                            new Vector2((geheelAantalmalen16 - 1) * 16, this.player.Position.Y);
                }
